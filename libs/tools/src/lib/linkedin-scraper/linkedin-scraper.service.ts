...
async function getDriver(profession: string): Promise<void> {
    ...
    try {
        ...
        await userInput.sendKeys('devrenatorodrigues@gmail.com');

        const passwordInput = await driver.findElement(By.id('password'));
        await passwordInput.sendKeys('Cs4ever1305@', Key.ENTER);
        ...
        const buttonNext = await driver.findElements(By.className('artdeco-pagination__button'));



        //TODO: LOOP TO GET ALL PAGES
        for (const button of buttonNext) {
            const buttonName = await button.getText();
            ...
            if (buttonName === 'Next') {
                ...
                const promises = structInfos(profilesList);

                const personArray = await Promise.all(promises)
...
getDriver('Programador em são paulo');
